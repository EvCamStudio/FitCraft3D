...
import StudioVisualizer, { type DecalConfig } from '../components/StudioVisualizer';

interface StudioPageProps {
  onNavigate: (page: 'landing' | 'products' | 'studio', model?: 'hoodie' | 'tshirt' | 'sweater') => void;
  initialModel: 'hoodie' | 'tshirt' | 'sweater';
}
...
export default function StudioPage({ onNavigate, initialModel }: StudioPageProps) {
  // State
  const [garmentType, setGarmentType] = useState(initialModel);
  const [fabric, setFabric] = useState<'cotton' | 'fleece'>('cotton');
  const [size, setSize] = useState('M');
  const [colors, setColors] = useState({ body: '#1b2e3c', sleeves: '#1b2e3c', collar: '#1b2e3c' });
  const [colorsName, setColorsName] = useState({ body: 'Tech Navy (Tren SaaS)', sleeves: 'Tech Navy (Tren SaaS)', collar: 'Tech Navy (Tren SaaS)' });
  ...
  const handleExportComplete = (dataUrl: string | null) => {
    ...
    } else if (pendingSave) setPendingSave(false);
  };
    ...
    <div className="h-screen flex flex-col bg-[#08090a] overflow-hidden">
      ...
      <header className="h-14 flex items-center justify-between px-5 border-b border-white/[0.06] bg-[#08090a]/95 backdrop-blur-xl z-50 shrink-0">
        <div className="flex items-center gap-4">
          <button onClick={() => onNavigate('landing')} className="flex items-center gap-2 text-sm text-[#9ba3af] hover:text-[#2de295] transition-colors">
          ...
          <div className="flex items-center gap-2.5">
            ...
            <span className="font-[var(--font-display)] font-bold text-sm tracking-tight">FITCRAFT <span className="text-[#2de295]">3D</span></span>
          </div>
        </div>
        ...
        <div className="flex items-center gap-3">
          <button onClick={() => setShowCheckout(true)} className="btn-primary text-xs py-2 px-4">
          ...
          <button className="w-8 h-8 rounded-lg border border-white/[0.08] flex items-center justify-center text-[#9ba3af] hover:text-[#2de295] hover:border-[#2de295]/30 transition-all">
      ...
      <div className="flex-1 flex overflow-hidden min-h-0">
        ...
        <aside className="w-[260px] shrink-0 border-r border-white/[0.06] bg-[#0c0f12] flex flex-col overflow-hidden">
          <div className="flex-1 overflow-y-auto p-4 space-y-4">
            ...
            <div>
              ...
              <div className="glass-card rounded-xl p-3 flex items-center gap-3">
                ...
                <div className="flex-1 min-w-0">
                  <div className="text-sm font-bold truncate">{activeModel.name}</div>
                  <div className="text-[10px] text-[#9ba3af]">PBR v4 · {activeModel.category}</div>
                </div>
                <button onClick={() => onNavigate('products')} className="text-[10px] font-bold text-[#2de295] bg-[#2de295]/10 px-2 py-1 rounded-md hover:bg-[#2de295]/20 transition-colors">
                  Ubah
                </button>
              </div>
            </div>
              ...
              <div className="grid grid-cols-2 gap-2">
                {[
                  ...
                  <button
                    ...
                    className={`p-3 rounded-xl border text-left transition-all ${fabric === mat.id
                      ? 'border-[#2de295]/40 bg-[#2de295]/10'
            ...
            <div>
              ...
              <div className="flex gap-1.5">
                {['S', 'M', 'L', 'XL', 'XXL'].map(sz => (
                  <button
                    key={sz}
                    onClick={() => setSize(sz)}
                    className={`flex-1 py-2 rounded-lg text-xs font-bold transition-all ${size === sz
              ...
              <button onClick={() => setShowSizeGuide(true)} className="mt-2 w-full py-2 text-[10px] font-bold text-[#9ba3af] border border-dashed border-white/[0.08] rounded-lg hover:border-[#2de295]/30 hover:text-[#2de295] transition-all">
            ...
            <div>
              ...
              <button
                ...
                className="w-full glass-card rounded-xl p-3 text-left hover:border-[#2de295]/30 transition-all group"
              >
          ...
          <div className="p-3 border-t border-white/[0.06]">
            <button onClick={() => setShowSizeGuide(true)} className="flex items-center gap-2 text-xs text-[#9ba3af] hover:text-[#2de295] transition-colors">
        ...
        <section className="flex-1 relative bg-gradient-to-b from-[#0c0f12] to-[#08090a] min-w-0">
          ...
          {/* Floating info */}
          <div className="absolute top-5 right-5 z-20 glass-panel rounded-xl px-4 py-3">
          ...
          <div className="absolute top-5 left-1/2 -translate-x-1/2 z-20 glass-panel rounded-full px-2 py-1.5 flex gap-1">
            {[
              ...
              <button key={i} className={`w-8 h-8 rounded-full flex items-center justify-center transition-all ${i === 0 ? 'bg-[#2de295]/20 text-[#2de295]' : 'text-[#4b5563] hover:text-[#9ba3af]'}`} title={view.label}>
            ...
            <div className="glass-panel rounded-2xl px-3 py-2 flex items-center gap-1">
              {[
                ...
                <button
                  ...
                  className={`flex flex-col items-center gap-1 px-3 py-1.5 rounded-xl transition-all ${lightingPreset === preset.id
              ...
              {[
                ...
                <button
                  key={tool.id}
                  onClick={tool.action}
                  className={`flex flex-col items-center gap-1 px-3 py-1.5 rounded-xl transition-all ${tool.active
                    ? 'bg-[#2de295]/10 text-[#2de295]'
                    : 'text-[#9ba3af] hover:text-[#f0f2f5] hover:bg-white/[0.04]'
                    }`}
                >
        ...
        {/* RIGHT SIDEBAR */}
        <aside className="w-[300px] shrink-0 border-l border-white/[0.06] bg-[#0c0f12] flex flex-col overflow-hidden">
          {/* Tabs */}
          <div className="flex gap-1 p-2 border-b border-white/[0.06] bg-white/[0.02]">
            {[
              ...
              <button
                ...
                className={`flex-1 py-2.5 rounded-lg text-[11px] font-bold transition-all ${activeTab === tab.id
              ...
              <div className="space-y-4">
                ...
                {decal.type === 'custom' && decal.customImage && (
                  <div className="flex items-center justify-between p-2.5 bg-[#2de295]/5 rounded-lg border border-[#2de295]/20">
                    ...
                    <button onClick={() => setDecal(prev => ({ ...prev, type: 'preset', presetName: 'fitcraft', customImage: null, customImageName: '' }))} className="text-[#ef4444] text-xs font-bold hover:scale-110 transition-transform">
                      ✕
                    </button>
                  </div>
                )}
                  ...
                  <div className="grid grid-cols-3 gap-2">
                    {[
                      ...
                      <button
                        ...
                        className={`flex flex-col items-center gap-1.5 p-3 rounded-xl border transition-all ${decal.type === 'preset' && decal.presetName === preset.id
                  ...
                  <div className="flex flex-wrap gap-2">
                    {presetColorOptions.map(opt => (
                      <button
                        ...
                        className={`w-7 h-7 rounded-full border-2 transition-all ${colors.body === opt.hex
                ...
                <div>
                  <div className="flex items-center justify-between mb-2">
                    ...
                    <button onClick={triggerSaveDesign} className="text-[10px] font-bold text-[#2de295] bg-[#2de295]/10 px-2 py-1 rounded-md hover:bg-[#2de295]/20 transition-colors">
                      + Simpan
                    </button>
                  </div>
                      ...
                      {savedDesigns.map(ds => (
                        <div key={ds.id} className="glass-card rounded-xl p-2 cursor-pointer group relative" onClick={() => handleLoadDesign(ds)}>
                          <button
                            ...
                            className="absolute -top-1 -right-1 w-5 h-5 rounded-full bg-[#ef4444]/20 text-[#ef4444] flex items-center justify-center text-[10px] font-bold opacity-0 group-hover:opacity-100 transition-opacity hover:bg-[#ef4444] hover:text-white"
                          >
                            ✕
                          </button>
                  ...
                  <div className="flex gap-2">
                    {[
                      ...
                      <button
                        ...
                        className={`flex-1 py-2 rounded-lg text-[10px] font-bold transition-all ${decal.textColor === c.value
                          ? 'ring-2 ring-[#2de295]'
                          : ''
          ...
          <div className="p-4 border-t border-white/[0.06] bg-white/[0.02]">
            ...
            <button onClick={() => setShowCheckout(true)} className="w-full btn-primary justify-center py-3">
              PESAN DESAIN INI
        ...
        {showCheckout && (
          ...
          >
            <motion.div
              ...
              className="bg-[#0c0f12] border border-white/[0.08] rounded-2xl p-6 max-w-md w-full max-h-[90vh] overflow-y-auto"
            >
              <div className="flex items-center justify-between mb-4">
                <h2 className="heading-md text-base"> Ringkasan Desain</h2>
                <button onClick={() => setShowCheckout(false)} className="w-8 h-8 rounded-lg bg-white/[0.04] flex items-center justify-center text-[#9ba3af] hover:text-[#f0f2f5] transition-colors">✕</button>
              </div>
        ...
        {showSizeGuide && (
          ...
          >
            <motion.div
              ...
              exit={{ scale: 0.95, opacity: 0 }}
              onClick={e => e.stopPropagation()}
              className="bg-[#0c0f12] border border-white/[0.08] rounded-2xl p-6 max-w-md w-full"
            >
              <div className="flex items-center justify-between mb-4">
                <h2 className="heading-md text-base">📏 Panduan Ukuran</h2>
                <button onClick={() => setShowSizeGuide(false)} className="w-8 h-8 rounded-lg bg-white/[0.04] flex items-center justify-center text-[#9ba3af] hover:text-[#f0f2f5]">✕</button>
              </div>
        ...
        {showSuccess && (
          ...
          >
            <motion.div
              ...
              exit={{ scale: 0.95, opacity: 0 }}
              onClick={e => e.stopPropagation()}
              className="bg-[#0c0f12] border border-white/[0.08] rounded-2xl p-8 max-w-sm w-full text-center"
            >
              ...
              <h2 className="heading-md text-lg mb-2">Pesanan Berhasil!</h2>
              ...
              <button onClick={() => setShowSuccess(false)} className="btn-primary w-full justify-center">
                Selesai & Kembali
              </button>
            </motion.div>
          </motion.div>
